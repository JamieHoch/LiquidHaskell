...
{-@ insTree :: t:BiTree a -> ts:[BiTree a] 
        -> {ti:(Tick {zs:[BiTree a]| len zs <= len ts + 1}) | tcost ti + pot (tval (insTree t ts)) - pot ts == 2 && pot (tval (insTree t ts)) - pot ts <= 1} @-}
...
{-@ mergeHeap :: Ord a => ts1:[BiTree a] -> ts2:[BiTree a] 
        -> {ti:Tick [BiTree a] | tcost ti + pot (tval (mergeHeap ts1 ts2)) - (pot ts1 + pot ts2) <= pot (tval (mergeHeap ts1 ts2)) && pot (tval (mergeHeap ts1 ts2)) == len (tval (mergeHeap ts1 ts2)) && len (tval (mergeHeap ts1 ts2)) <= len ts1 + len ts2} @-}
...
mergeHeap ts1@(t1:ts1') ts2@(t2:ts2')
    ...
    | otherwise = waitN 2 (tval (insTree (link t1 t2) (tval (mergeHeap ts1' ts2')))) -- !!CHEAT
    -- insTree (link t1 t2) RTick.=<< mergeHeap ts1' ts2' -- worst case not amortized
...
{-@ removeMinTree :: Ord a => ts:NEHeap a
        -> {ti:Tick (BiTree a, [BiTree a]) | tcost ti + pott (tval ti) - pot ts <= pot ts && pott (tval ti) == pot ts && tcost ti <= pot ts} @-}
...
{-@ deleteMin' :: Ord a => ts1:[BiTree a] -> ts2:[BiTree a] 
        -> {h:NEHeap a | pot h == pot ts2 + 1} -> {ti:Tick (Heap a) | tcost ti + pot (tval (deleteMin' ts1 ts2 h)) - pot h <= 2* (pot ts1 + pot ts2) && pot (tval (deleteMin' ts1 ts2 h)) <= pot ts1 + pot ts2} @-}
