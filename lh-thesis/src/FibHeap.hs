...
{-@ link :: t1:FibTree a -> {t2:FibTree a | rank t1 == rank t2} -> {v:FibTree a | rank v == 1 + (rank t1)} @-}
...
{-@ meld :: [FibTree a] -> FibTree a -> {t:[FibTree a] | len t > 0}@-}
meld :: Ord a => [FibTree a] -> FibTree a -> [FibTree a]
meld [] t = [t]
meld (t':ts) t = if rank t' == rank t then meld ts (link t t')
                     else t:t':ts

{-@ meld' :: {t:[FibTree a] | len t > 0} -> [FibTree a] -> {t:[FibTree a] | len t > 0} @-}
meld' :: Ord a => [FibTree a] -> [FibTree a] -> [FibTree a]
meld' (t:ts) [] = (t:ts)
